...
%Definir que una ficha de un policía capture a una ficha del ladrón.

Diremos que la $i$-\'esima ficha de $R$, $r_i$, est\'a \textbf{bajo asedio} en
el turno $t$ si cada ficha de su $k-1$ conjunto asociado, $\overline{c_i}$,
est\'a capturando la ficha $r_i$ (en el turno $t$), es decir, $c_{1,i}, \dots,
c_{i-1,i}, c_{i+1,i}, \dots, c_{k,i}$ capturan a $r_i$. De forma similar,
decimos que $R$ est\'a \textbf{bajo asedio} en el turno $t$ si cada una de sus
fichas lo est\'a (en el mismo turno $t$).
...
\begin{proof}
    ...
    i$, mientras que su ficha libre no se mover\'a (salvo por aplicaciones
    d\cref{alg:desob}) hasta que el resto de sus fichas se encuentre capturando
    a su objetivo y esta no est\'e obstruida.

    As\'i pues, dada una instancia $I(t)$ y para cada $i\in\seg{1}{k}$, si $c_i$
    tiene al menos una ficha distinta de su ficha libre, $c_{i,j}$, tal que esta
    no se encuentra capturando a su objetivo $r_j$, entonces $c_i$ procede a
    mover la ficha de este tipo cuyo \'indice $j$-\'esimo sea m\'inimo,
    resolviendo obstrucciones con \cref{alg:desob} en caso de ser necesario.
    Esto implica que, en su turno, cada uno de los polic\'ias redujo la suma de
    las distancias de sus fichas a sus objetivos en al menos $1$. Luego, si $R$
    mueve su ficha $r_j$ (o pasa), esta se aleja a lo m\'as $1$ de distancia de
    las $k-1$ fichas que buscan capturarla (todas las $c_{i,j}$ con $j\neq i$),
    mientras que, como $c_j$ ignora a $r_j$, posterior al turno de $R$ se tiene
    que
    \[
...
\begin{algorithm}[h!]
    \SetAlgorithmName{Algoritmo}{}
      ...
      de sus fichas.} \KwOut{Una instancia $I(t)$ en la que $C$ captura a $R$.}
      \BlankLine
      define $I(0)$ como la instancia en la que $C$ y $R$ se encuentran en su
      configuraci\'on inicial\;
      $t\leftarrow 0$\;
      \While{$C$ no est\'e capturando a $R$ en $I(t)$}{
        \ForEach{$1\leq i\leq k$}{
            \If{existe $j\neq i$ tal que $c_{i,j}$ no est\'a capturando a $r_j$}{
                define $j$ como el m\'inimo \'indice tal que $c_{i,j}$ no est\'a 
                capturando a $r_j$\;
                define $v_{i,j}$ como el segundo v\'ertice en la
                $c_{i,j}(t)r_j(t)$-trayectoria\;
                \If{existe $s$ tal que $c_{i,s}(t) = v_{i,j}$}{
                    \If{$s=i$ y $v_{i,j}=r_j(t)$}{
                        define $c_{i}(t+1)$ como la configuraci\'on $c_i(t)$ intercambiando las
                        etiquetas de $c_{i,s}$ y $c_{i,j}$\;
                    }\Else{
                        $\textsc{Desobstrucci\'on}
                        \paren{k,S_n,I(t),c_{i,s},c_{i,j}}\leftarrow c_{i}(t+1)$\;
                    }
                }\Else{
                    define $c_i(t+1)$ como la configuraci\'on $c_i(t)$ cambiando el 
                    elemento $(c_{i,j},c_{i,j}(t))$ por $(c_{i,j},v_{i,j})$\;
                }
            }\Else{
                define $v_{i,i}$ como el segundo v\'ertice en la
                $c_{i,i}(t)r_i(t)$-trayectoria\;
                \If{existe $s$ tal que $c_{i,s}(t) = v_{i,i}$}{
                    define $c_{i}(t+1)$ como la misma configuraci\'on $c_i(t)$\;
                }\Else{
                    define $c_i(t+1)$ como la configuraci\'on $c_i(t)$ cambiando el 
                    elemento $(c_{i,i},c_{i,i}(t))$ por $(c_{i,i},v_{i,i})$\;
                }
            }
        }
        define $I(t+1)$ como la uni\'on de los $c_i(t+1)$ y los $R(t+1)$, donde
        este \'ultimo es una configuraci\'on de $R$ obtenida legalmente de
        $R(t)\subseteq I(t)$\;
        $t+1\leftarrow t$\;
      }
      {\Return $I(t)$}
