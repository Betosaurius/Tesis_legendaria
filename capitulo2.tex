...
\begin{proof}
    ...
    d\cref{alg:desob}) hasta que todas las dem\'as fichas de $c_i$ capturen a
    sus objetivos. Se ver\'a que, en cada turno, $C$ puede mantener el n\'umero
    de fichas del ladr\'on bajo asedio y, despu\'es de un cierto n\'umero de
    turnos, puede aumentar la cantidad de fichas de $R$ bajo asedio.

    Para la primera afirmaci\'on, si $R$ mueve una ficha bajo asedio $r_i$,
    entonces las fichas en $\overline{c_i}$ proceden a perseguir al ladr\'on a
    donde sea que se mueva. Puede suceder que en la nueva posici\'on de $r_i$ se
    encuentren algunas de las fichas (posiblemente todas) de los polic\'ias que
    tienen una ficha en $\overline{c_i}$. En este caso, solo las fichas que no
    esten obstruidas se mueven a la nueva posici\'on de $r_i$, de manera que
    para el resto se ejecuta \cref{alg:desob}, por lo que, para cada polic\'ia,
    sucede que la suma de las distancias de sus fichas a sus objetivos se reduce
    y $r_i$ vuelve a estar bajo asedio.
    ...
    sus objetivos se redujo \'unicamente con reetiquetados. Es posible que
    \cref{alg:desob} se ejecute varias veces en un mismo turno sin mover fichas
    de $c_i$.   En tal caso, \cref{alg:desob} \'unicamente realiza una
    sucesi\'on de reetiquetaciones que puede culminar en el movimiento de alguna
    ficha de $c_i$, o en el caso en que cada ficha de $c_i$ captura a su
    objetivo.  El \'ultimo caso s\'olo sucede cuando inicialmente $c_i$ ya
    estaba capturando a $R$ (pero sus fichas no capturaban a sus
    correspondientes objetivos), por lo que $R$ no jug\'o \'optimamente.
